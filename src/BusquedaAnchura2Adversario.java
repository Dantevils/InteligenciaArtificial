...
/*Busqueda Anchura del Adversario*/
public class BusquedaAnchura2Adversario extends TimerTask implements Constantes {
...
public BusquedaAnchura2Adversario(Laberinto laberinto, Adversario adversario) {
...
private void moverArriba(Estado e) {
    if ( e.y > 0 ) {
        if ( laberinto.celdas[e.x][e.y-1].tipo != 'O' && laberinto.celdas[e.x][e.y-1].tipo != 'P' && laberinto.celdas[e.x][e.y-1].tipo != 'J' ) {
...
private void moverIzquierda(Estado e) {
    if ( e.x > 0 ) {
        if ( laberinto.celdas[e.x-1][e.y].tipo != 'O' && laberinto.celdas[e.x-1][e.y].tipo != 'P' && laberinto.celdas[e.x-1][e.y].tipo != 'J') {
...
public synchronized void run() {
    ...
    if (!parar) {
        ...
        subinicial = new Estado(adversario.adversario.x,adversario.adversario.y ,'N',null);
       ...
       // subobjetivo = new Estado(laberinto.LienzoPadre.Hero.jugador.x,laberinto.LienzoPadre.Hero.jugador.y,'N',null);
        ...
        if (pasos.size()> 1) {
            switch(pasos.get(1)) {
            case 'D':laberinto.LienzoPadre.adversario.moverCeldaAbajo();;break;
            case 'U':laberinto.LienzoPadre.adversario.moverCeldaArriba();break;
            case 'R':laberinto.LienzoPadre.adversario.moverCeldaDerecha();break;
            case 'L':laberinto.LienzoPadre.adversario.moverCeldaIzquierda();break;
